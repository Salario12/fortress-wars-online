...
wss.on("connection",ws=>{
  ...
  ws.on("message",raw=>{
    ...
    if(m.type==="create_room"){
      if(player.room)leave(player);const id=code();const room={id,players:[player],created:Date.now()};
    ...
    if(m.type==="join_room"){
      ...
      if(player.room)leave(player);player.room=id;player.name=String(m.name||"Jugador 2").slice(0,24);room.players.push(player);
      broadcast(room,{type:"room_ready",room:id,players:room.players.map(p=>({id:p.id,name:p.name}))});return;
    }
...
function leave(player){if(!player.room)return;const room=rooms.get(player.room);player.room=null;if(!room)return;room.players=room.players.filter(p=>p!==player);if(!room.players.length)rooms.delete(room.id);else broadcast(room,{type:"player_left",playerId:player.id});}
